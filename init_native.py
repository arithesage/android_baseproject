...
class PLACEHOLDERS:
    PROJECT_NAME_PLACEHOLDER = "BaseNativeApp"
    PACKAGE_PLACEHOLDER = "me.android.basenativeapp"
...
def init_project (package_name):
    ...
    if not replace_all_in (PLACEHOLDERS.PROJECT_NAME_PLACEHOLDER.lower(), project_name_lowercase, main_src):
        echo (MESSAGES.Failed)
        echo (MESSAGES.InitializationFailed)
        abort ()
    ...
    echo_va ("Updating $[0] and $[1]...", FILES.CMakeLists_TXT, FILES.NativeLib_CPP)

    if not replace_all_in (PLACEHOLDERS.PROJECT_NAME_PLACEHOLDER.lower(), project_name_lowercase, make_path (PATHS.Main_CPP, FILES.CMakeLists_TXT)) \
    or not replace_all_in (PLACEHOLDERS.PROJECT_NAME_PLACEHOLDER.lower(), project_name_lowercase, make_path (PATHS.Main_CPP, FILES.NativeLib_CPP)):
        echo (MESSAGES.Failed)
        echo (MESSAGES.InitializationFailed)
        abort ()
